...
```
[국립중앙의료원 API]      [HIRA API]
         ↓                    ↓
    collect.py (cron 매 정각 자동 실행)
         ↓
┌──────────────────────────────────┐
│      OCI Block Volume (BV-14)    │  ← emergency_raw.csv
│      마운트 경로: /mnt/iscsi      │
└──────────────────────────────────┘
         ↓
    preprocess.py (전처리 + DB 적재)
         ↓
┌─────────────────┐   ┌──────────────────────────┐
│  Oracle DB XE   │   │   OCI Object Storage      │
│ emergency_bed   │   │  bucket-14-cbnu-lv2       │
└─────────────────┘   └──────────────────────────┘
         ↓
    Flask REST API (port 80)
         ↓
    React + Chart.js (port 3000)
         ↓
    브라우저: http://131.186.18.12:3000
```
...
**향후 개선 방향**
- 경남 수집 실패 예외 처리 강화
- 날짜별 탭 네비게이션 기능 데이터 연동 (UI는 구현 완료)
